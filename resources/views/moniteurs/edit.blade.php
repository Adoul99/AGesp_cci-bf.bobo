...
<div class="form-page">
    <div class="page-header-card">
        <h1 class="page-title">| Modifier Moniteur</h1>
        ...
        <form method="POST" action="{{ route('moniteurs.update', $moniteur->id) }}">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="nom" class="form-input" value="{{ old('nom', $moniteur->nom) }}" required>
                    @error('nom')<span class="field-error">{{ $message }}</span>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Prénom <span class="required-star">*</span></label>
                    <input type="text" name="prenom" class="form-input" value="{{ old('prenom', $moniteur->prenom) }}" required>
                    @error('prenom')<span class="field-error">{{ $message }}</span>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Téléphone</label>
                    <input type="text" name="telephone" class="form-input" value="{{ old('telephone', $moniteur->telephone) }}">
                    @error('telephone')<span class="field-error">{{ $message }}</span>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Email</label>
                    <input type="email" name="email" class="form-input" value="{{ old('email', $moniteur->email) }}">
                    @error('email')<span class="field-error">{{ $message }}</span>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Spécialité</label>
                    <input type="text" name="specialite" class="form-input" value="{{ old('specialite', $moniteur->specialite) }}">
                ...
                <div class="form-group">
                    ...
                    <select name="disponibilite" class="form-select">
                        <option value="1" {{ old('disponibilite', $moniteur->disponibilite) == '1' ? 'selected' : '' }}>✓ Disponible</option>
                        <option value="0" {{ old('disponibilite', $moniteur->disponibilite) == '0' ? 'selected' : '' }}>✕ Indisponible</option>
                    </select>
