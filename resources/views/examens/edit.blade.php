<x-layouts::app.sidebar title="Modifier Examen">
    ...
    <div class="content-wrapper" style="padding: 2rem;">
        ...
        <!-- Formulaire -->
        <form method="POST" action="{{ route('examens.update', $examen->id) }}" style="background: white; padding: 2rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md); border: 1px solid var(--color-gray-100);">
            @csrf
            @method('PUT')
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               id="libelle"
                               name="libelle" 
                               value="{{ old('libelle', $examen->libelle) }}"
                    ...
                    <div class="form-group">
                        ...
                        <select id="statutExamen"
                            ...
                            <option value="admis" {{ old('statutExamen', $examen->statutExamen) == 'admis' ? 'selected' : '' }}>🟢 Admis</option>
                            <option value="ajourne" {{ old('statutExamen', $examen->statutExamen) == 'ajourne' ? 'selected' : '' }}>🔴 Ajourné</option>
                            <option value="abandon" {{ old('statutExamen', $examen->statutExamen) == 'abandon' ? 'selected' : '' }}>⚪ Abandon</option>
                        </select>
                        @error('statutExamen')
                            <span style="color: var(--color-red); font-size: 0.75rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" 
                               id="dateDebut"
                               name="dateDebut" 
                               value="{{ old('dateDebut', $examen->dateDebut) }}"
                               style="width: 100%; padding: 0.75rem 1rem; border: 2px solid var(--color-gray-200); border-radius: var(--radius-md); font-size: 0.875rem; font-family: inherit; transition: all var(--transition-normal); color: var(--color-dark);"
                               onfocus="this.style.borderColor='var(--color-green)'; this.style.boxShadow='0 0 0 3px rgba(0, 122, 94, 0.1)'"
                               onblur="this.style.borderColor='var(--color-gray-200)'; this.style.boxShadow='none'"
                               required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" 
                               id="dateFin"
                               name="dateFin" 
                               value="{{ old('dateFin', $examen->dateFin) }}"
                               style="width: 100%; padding: 0.75rem 1rem; border: 2px solid var(--color-gray-200); border-radius: var(--radius-md); font-size: 0.875rem; font-family: inherit; transition: all var(--transition-normal); color: var(--color-dark);"
                               onfocus="this.style.borderColor='var(--color-green)'; this.style.boxShadow='0 0 0 3px rgba(0, 122, 94, 0.1)'"
                               onblur="this.style.borderColor='var(--color-gray-200)'; this.style.boxShadow='none'"
                               required>
                    ...
                    <div class="form-group">
                        ...
                        <select id="moniteur_id"
                            ...
                            @foreach($moniteurs as $moniteur)
                                <option value="{{ $moniteur->id }}" {{ old('moniteur_id', $examen->moniteur_id) == $moniteur->id ? 'selected' : '' }}>
        ...
        <div style="margin-top: 2rem; padding: 1rem; background: rgba(0, 122, 94, 0.1); border-left: 4px solid var(--color-green); border-radius: var(--radius-md); color: var(--color-green-dark); font-size: 0.875rem;">
            <strong>ℹ️ Information :</strong> Les modifications d'un examen déjà programmé doivent être validées avec précaution afin de ne pas impacter les candidats associés.
        </div>
    </div>
