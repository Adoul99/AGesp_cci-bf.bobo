<x-layouts::app title="Nouveau Dossier">
    <style>
        :root {
    ...
    <div class="content-wrapper" style="padding: 2rem;">
        ...
        @if ($errors->any())
            <div style="background: #f8d7da; color: #721c24; padding: 1.5rem; border-radius: var(--radius-lg); margin-bottom: 2rem; border: 1px solid #f5c6cb;">
                <h4 style="margin: 0 0 0.5rem 0;">Erreurs détectées :</h4>
        ...
        <form method="POST" action="{{ route('dossiers.store') }}" enctype="multipart/form-data" style="background: white; padding: 2rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md);">
            @csrf
            ...
            <div style="margin-bottom: 2.5rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nomDossier" id="nomDossier" style="width: 100%; padding: 0.75rem; border: 2px solid var(--color-gray-200); border-radius: var(--radius-md);" required>
                    </div>

                    <div class="form-group">
                        <label for="dateDepot" style="display: block; margin-bottom: 0.5rem; font-weight: 600; font-size: 0.875rem; text-transform: uppercase;">Date de Dépôt *</label>
                        <input type="date" name="dateDepot" id="dateDepot" style="width: 100%; padding: 0.75rem; border: 2px solid var(--color-gray-200); border-radius: var(--radius-md);" required>
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 2.5rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                    @foreach(['cnib' => 'CNIB', 'photo_identite' => 'Photo Identité', 'certificat_medical' => 'Certificat Médical', 'acte_naissance' => 'Acte de Naissance', 'recu_paiement' => 'Reçu de Paiement', 'permis_c' => 'Copie Permis C'] as $field => $label)
                        <div class="form-group">
                            <label for="{{ $field }}" style="display: block; margin-bottom: 0.5rem; font-weight: 600; font-size: 0.875rem;">{{ $label }} *</label>
                            <input type="file" name="{{ $field }}" id="{{ $field }}" style="width: 100%; padding: 0.5rem; border: 1px dashed var(--color-gray-200); border-radius: var(--radius-md);" required>
                        </div>
                    @endforeach
                </div>
            </div>
