<x-layouts::app.sidebar title="Modifier Évaluation">
    ...
    <div class="content-wrapper" style="padding: 2rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem;">
                    ...
                    <div class="form-group">
                        ...
                        <select id="resultat"
                            ...
                            <option value="Admis" {{ old('resultat', $evaluation->resultat) == 'Admis' ? 'selected' : '' }}>Admis</option>
                            <option value="Ajourné" {{ old('resultat', $evaluation->resultat) == 'Ajourné' ? 'selected' : '' }}>Ajourné</option>
                            <option value="Absent" {{ old('resultat', $evaluation->resultat) == 'Absent' ? 'selected' : '' }}>Absent</option>
                            <option value="Eliminé" {{ old('resultat', $evaluation->resultat) == 'Eliminé' ? 'selected' : '' }}>Éliminé</option>
                        </select>
                        @error('resultat')
                            <span style="color: var(--color-red); font-size: 0.75rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                        @enderror
        ...
        <div style="margin-top: 2rem; padding: 1rem; background: rgba(0, 122, 94, 0.1); border-left: 4px solid var(--color-green); border-radius: var(--radius-md); color: var(--color-green-dark); font-size: 0.875rem;">
            <strong>ℹ️ Information :</strong> Les champs marqués avec un <span style="color: var(--color-red); font-weight: bold;">*</span> sont obligatoires. Les modifications d'une évaluation doivent être gérées avec précaution.
        </div>
    </div>
