<x-layouts::app.sidebar title="Modifier Véhicule">
    <style>
        :root {
    ...
    <div class="content-wrapper" style="padding: 2rem; max-width: 800px; margin: 0 auto;">
        <div class="header-section" style="margin-bottom: 2rem; background: white; padding: 1.5rem 2rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md); border-left: 4px solid var(--color-red);">
            <h1 style="font-size: 1.8rem; font-weight: 700; color: var(--color-dark); margin: 0; display: flex; align-items: center;">
                ...
                Modifier : {{ $vehicule->nomVehicule }}
            </h1>
            <p style="color: #666; margin: 0.5rem 0 0 1.5rem;">Mettez à jour les informations du véhicule sélectionné.</p>
        </div>

        <form method="POST" action="{{ route('vehicules.update', $vehicule->id) }}" style="background: white; padding: 2rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md);">
            @csrf @method('PUT')
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div style="grid-column: span 2;">
                    <label style="display: block; font-weight: 600; color: var(--color-dark); margin-bottom: 0.5rem;">Nom Véhicule</label>
                    <input type="text" name="nomVehicule" value="{{ old('nomVehicule', $vehicule->nomVehicule) }}" style="width: 100%; padding: 0.75rem; border: 1px solid var(--color-gray-100); border-radius: 8px; outline: none;" required>
                </div>
                <div>
                    <label style="display: block; font-weight: 600; color: var(--color-dark); margin-bottom: 0.5rem;">Modèle</label>
                    <input type="text" name="modeleVehicule" value="{{ old('modeleVehicule', $vehicule->modeleVehicule) }}" style="width: 100%; padding: 0.75rem; border: 1px solid var(--color-gray-100); border-radius: 8px; outline: none;" required>
                </div>
                <div>
                    <label style="display: block; font-weight: 600; color: var(--color-dark); margin-bottom: 0.5rem;">Immatriculation</label>
                    <input type="text" name="immatriculation" value="{{ old('immatriculation', $vehicule->immatriculation) }}" style="width: 100%; padding: 0.75rem; border: 1px solid var(--color-gray-100); border-radius: 8px; outline: none;" required>
                </div>
            </div>
