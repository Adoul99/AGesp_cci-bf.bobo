<x-layouts::app.sidebar title="Liste des Groupes">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Liste des Groupes</h1>
            <a href="{{ route('groupes.create') }}" 
               class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                + Nouveau Groupe
            </a>
        </div>

        <table class="w-full border-collapse border border-gray-300">
            <thead class="bg-gray-100">
                <tr>
                    <th class="border p-3 text-left">Nom Groupe</th>
                    <th class="border p-3 text-left">Date Début Formation</th>
                    <th class="border p-3 text-left">Candidats</th>
                    <th class="border p-3 text-left">Actions</th>
                </tr>
            </thead>
            <tbody>
                @foreach($groupes as $groupe)
                <tr class="hover:bg-gray-50">
                    <td class="border p-3">{{ $groupe->nomGroupe }}</td>
                    <td class="border p-3">{{ $groupe->dateDebutFormation }}</td>
                    <td class="border p-3">
                        @foreach($groupe->candidats as $candidat)
                            <div class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-sm mb-1">
                                {{ $candidat->nom }} {{ $candidat->prenom }}
                            </div>
                        @endforeach
                    </td>
                    <td class="border p-3">
                        <div class="flex gap-2">
                            <a href="{{ route('groupes.edit', $groupe->id) }}" 
                               class="bg-yellow-500 text-white px-3 py-1 rounded">
                                Modifier
                            </a>
                            <form method="POST" action="{{ route('groupes.destroy', $groupe->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded">
                                    Supprimer
                                </button>
                            </form>
                        </div>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
    </div>
</x-layouts::app.sidebar>