<x-layouts::app.sidebar title="Nouvelle Programmation">
    <style>
        ...
        .prog-primary { background: linear-gradient(135deg, var(--color-red) 0%, #B91C1C 100%); color: white; }
    ...
    <div class="p-6">
        <div class="prog-card p-8 mb-6">
            <div class="mb-6">
                <h1 class="prog-title">Nouvelle programmation</h1>
                <p class="prog-subtitle">Créez une programmation claire, avec le même design que l’index.</p>
            </div>
            ...
            <form method="POST" action="{{ route('programmations.store') }}">
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div>
                        ...
                        <input id="dateDebut" type="date" name="dateDebut" value="{{ old('dateDebut') }}" class="prog-input" required>
                        @error('dateDebut')<p class="error-text">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="prog-label" for="dateFin">Date Fin</label>
                        <input id="dateFin" type="date" name="dateFin" value="{{ old('dateFin') }}" class="prog-input" required>
                        ...
                        <div class="border border-gray-200 rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto">
                            @foreach($candidats as $candidat)
                                <label class="flex items-center gap-3 cursor-pointer rounded-xl border border-transparent hover:border-gray-200 p-3 transition">
                                    <input type="checkbox" name="candidat_ids[]" value="{{ $candidat->id }}" class="prog-checkbox" {{ in_array($candidat->id, old('candidat_ids', [])) ? 'checked' : '' }}>
                ...
                <div class="mt-8 flex flex-col sm:flex-row sm:justify-end gap-4">
                    ...
                    <button type="submit" class="prog-button prog-primary w-full sm:w-auto">Enregistrer</button>
                </div>
            </form>
        </div>
    </div>
